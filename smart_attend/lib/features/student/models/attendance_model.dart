...
class DayAttendanceModel {
  ...
  const DayAttendanceModel({required this.date, required this.sessions});

  bool get hasClasses => sessions.isNotEmpty;
  bool get allPresent =>
      sessions.isNotEmpty &&
      sessions.every((s) => s.status == AttendanceStatus.present);
  bool get hasAbsence =>
      sessions.any((s) => s.status == AttendanceStatus.absent);
  bool get isUpcoming =>
      sessions.any((s) => s.status == AttendanceStatus.upcoming);
  bool get isCancelled =>
      sessions.isNotEmpty &&
      sessions.every((s) => s.status == AttendanceStatus.cancelled);
